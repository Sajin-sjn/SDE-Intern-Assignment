...
function ManageVideos() {
  ...
  useEffect(() => {
    if (!user) {
      console.log('[ManageVideos] No user, skipping fetch');
      return;
    }
    console.log('[ManageVideos] Fetching videos...');
    setLoading(true);
    ...
    api
      ...
      .catch((err) => {
        if (err.name === 'AbortError') {
          console.log('[ManageVideos] Fetch aborted');
        } else {
          console.error('[ManageVideos] Error fetching videos:', {
            message: err.message,
            status: err.response?.status,
    ...
    return () => {
      console.log('[ManageVideos] Aborting fetch on unmount');
      controller.abort();
    };
  }, [user]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log('[ManageVideos] Sending video data:', {
        title: formData.title,
        hasFile: !!formData.video_file,
      ...
      console.error('[ManageVideos] Error saving video:', {
        status: err.response?.status,
        data: err.response?.data,
      });
