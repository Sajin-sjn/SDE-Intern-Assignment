...
class VideoProgressViewSet(viewsets.ModelViewSet):
    ...
    def update_progress(self, request):
        ...
        with transaction.atomic():
            ...
            # Merge all intervals
            all_intervals = merge_intervals(existing_intervals + new_intervals)
            print(f"Video {video.id} - All intervals: {all_intervals}")  # Debug
            ...
            try:
                for start, end in all_intervals:
                    ...
                    interval_obj.clean()  # Validate interval before saving
                    interval_obj.save()
            except ValidationError as e:
                print(f"Video {video.id} - Validation error: {str(e)}")  # Debug
                return Response({
                    'error': f'Invalid intervals: {str(e)}'
                }, status=status.HTTP_400_BAD_REQUEST)
    ...
    def update(self, request, *args, **kwargs):
        return Response({
            'error': 'Use POST /api/update_progress/ for progress updates'
        }, status=status.HTTP_405_METHOD_NOT_ALLOWED)
